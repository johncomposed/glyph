...
async function shapeReducedFont(t, shapingSfnt, fontFile, shapingCasesDirectory) {
  const temporaryDirectory = await mkdtemp(join(tmpdir(), 'pmndrs-glyph-reduced-sfnt-'));
  ...
  await executeFile('cargo', [
    'run',
    '--manifest-path',
    fileURLToPath(new URL('../../../rust/font-baker/Cargo.toml', import.meta.url)),
    '--bin',
...
test('the canonical Inter fixture bakes deterministically and retains HarfRust shaping', async (t) => {
  ...
  assert.equal(first.report.shared.shaping.extentsAvailabilityBytes, expected.extentsAvailabilityBytes);
  assert.equal(
...
test('the authenticated Noto CJK fixture retains the closed shaping profile at the u16 limit', async (t) => {
  ...
  assert.deepEqual(
    ...
    ['BASE', 'GDEF', 'GPOS', 'GSUB', 'OS/2', 'VORG', 'cmap', 'head', 'hhea', 'hmtx', 'maxp', 'vhea', 'vmtx'],
  ...
  assert.deepEqual(first.report.transport, [{ artifactId: artifact.id, format: 'raw', bytes: expected.artifactBytes }]);
  ...
  assert.deepEqual(
    await shapeReducedFont(t, inspected.shapingSfnt, 'NotoSansCJKjp-Regular.shaping.otf', casesDirectory),
    JSON.parse(expectedOracleSource),
