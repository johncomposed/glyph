...
| 6.5   |   🟡   | Implement `@pmndrs/glyph/vue` as a thin TresJS reconciliation layer over the same retained `Text`/`TextGroup` lifecycle, with a Vite playground application.                                                                                                                                                                                                                                                                                                                                                                                                                   |  M   | 6.3, 11.7   |
...
| 12.2  |   ✅   | Implement staged run topology and direct engine-owned x/y occurrence placement, then cut semantic queries, publication, and renderer realization atomically to the single retained model. Preserve existing batching/draw topology, keep backend memory layout out of Codec authoring, and close the measured CPU/publication overhead before completion.                                                                                                                                                                                                                      |  XL  | 12.1        |
...
Item 12.1 used merged commit `2094243668bcf5462cff0ac3b1f7faf52cba3b6c` as its exact baseline. Items 12.1–12.5 are
complete. The direct engine-owned x/y occurrence offset is implemented through the generated semantic contract, Three,
and direct TypeGPU without changing batches or draws; public CPU semantic queries retain absolute origins while
positioned storage stays local. Fresh-main evidence is positive for ordinary Latin, justified Latin, dense CJK, and
mixed bidi after lazy absolute semantic materialization removed width-only semantic rewrites. Final two-pass candidate
medians are `1.562`, `2.229`, `2.277`, and `3.437 ms` respectively; mixed bidi is 13.0% faster than the recorded
fresh-main median. Both shader stacks pass all 120 Presentation cells across 10 workloads, WebGPU/WebGL2, and
Bitmap/MTSDF/Slug; the dedicated TypeGPU WebGPU, package, documentation, and reviewed release-size gates also pass. The
dependency does not include unrelated open work grouped under 11.18.
...
- [x] Fontations owns SFNT/TTC parsing and Skrifa owns maintained glyph bounds; fixtures cover invalid bytes, WOFF/WOFF2, required tables, AAT shaping systems, static-font axis requests, TTC face selection, and out-of-range face indexes. Variable sources pin one instance at bake time (D-371); the authenticated Oxanium `wght=700` fixture proves the reduced payload shapes exactly like HarfBuzz 13 at that instance.
...
- [x] `glyphs()` shares prepared analysis, broad shaping, and cached line plans with `measure()` while materializing paragraph-owned `fontHandles`, font slots, glyph IDs, UTF-16 clusters, font sizes, x/y positions, flags, and parallel line arrays only on demand.
- [x] Full layouts cache by complete normalized constraints. The renderer-free Paragraph retains at most three canonical measurement and positioned answers, covering unconstrained, at-most, and exact negotiation without unbounded glyph-array retention.
...
### 6.5 closure checklist

- [x] The Vue adapter retains one Three object per outer `Text`, exposes it as `instance` on the component ref, and forwards ordinary Object3D props to the Tres node without duplicating core behavior.
- [x] Nested `<Text>` slots flatten into one string plus ordered inherited spans; non-text children and box props on nested Text fail explicitly with the offending name.
- [x] `useFont`, `.preload`, `.clear`, and the format leaves share the Glyph resource graph and release mounted leases with the component scope; `handle.textCount` returns to zero after unmount.
- [x] One default root per `TresCanvas`; two canvases isolate roots and unmounting one leaves the other rendering.
...
caller-owned passes, using the shared `/shaders/typegpu` functions. The [hello-world app](../../../apps/typegpu-hello-world/README.md)
...
Items 11.1 through 11.7 are closed. `/shaders/tsl` publishes the stable native technique shaders; `/three/typegpu` publishes the
experimental adapters over `/shaders/typegpu`. Per D-345 and D-346, the stable and experimental Three configs remain separate while parity
...
- [x] verify the accepted stable-indirect removal with full ordered benchmarks, size, renderer gates, and Opus (D-362, D-364);
- [x] close the former dual-planner item by proving the ordered-only implementation needs no strategy dispatcher (D-362, D-364);
